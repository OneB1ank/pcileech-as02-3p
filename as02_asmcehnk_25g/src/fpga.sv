...
(
    ...
    // Exposed by asmcehnk_as02mc04_top in the same role as AMDUSB4 top params.
...
localparam AXIS_PCIE_CQ_USER_W = 88;
...
// UltraScale+ exposes four identical ready bits on RQ and CC.  Use bit 0
// explicitly instead of relying on an implicit vector-to-scalar truncation.
wire [3:0] pcie_axis_rq_tready;
wire [3:0] pcie_axis_cc_tready;

assign axis_pcie_rq.tready = pcie_axis_rq_tready[0];
assign axis_pcie_cc.tready = pcie_axis_cc_tready[0];

wire stat_err_cor;
wire stat_err_uncor;

assign stat_err_cor = 1'b0;
assign stat_err_uncor = 1'b0;
...
fpga_core #(
    ...
    // asmcehnk command/config-space identity and UDP transport
...
core_inst (
     ...
     * Synchronous reset
     */
    .clk_100mhz(clk_100mhz_int),
    .rst_100mhz(rst_100mhz_int),
