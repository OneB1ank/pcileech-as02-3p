`timescale 1ns/1ps
module tb_udp_rx;
reg clk=0; always #1.241 clk=~clk;
reg rst=1;
...
always @(posedge clk) if(com_rx_valid) begin rx_words[rx_count]=com_rx_data; rx_count=rx_count+1; end
initial begin
 ...
 for(fi=0;fi<6;fi=fi+1) begin
  ...
  while(bi<frame_len[fi]) begin
   @(negedge clk);rx_if.tdata=0;rx_if.tkeep=0;rx_if.tstrb=0;beat_bytes=(frame_len[fi]-bi>8)?8:(frame_len[fi]-bi);
   for(lane=0;lane<beat_bytes;lane=lane+1) begin rx_if.tdata[lane*8 +: 8]=frame_mem[fi][bi+lane];rx_if.tkeep[lane]=1;rx_if.tstrb[lane]=1;end
   rx_if.tlast=(bi+beat_bytes>=frame_len[fi]);rx_if.tvalid=1;@(posedge clk);while(!rx_if.tready) @(posedge clk);bi=bi+beat_bytes;
  end
  @(negedge clk);rx_if.tvalid=0;rx_if.tlast=0;rx_if.tkeep=0;repeat(40) @(posedge clk);
  if(fi==0||fi==5) begin
   if(rx_count-before_count!=2) begin $display("VALID_COUNT_FAIL frame=%0d delta=%0d",fi,rx_count-before_count);errors=errors+1;end
    if(rx_words[before_count]!==64'h0100010080022377||rx_words[before_count+1]!==64'h0000000001000377) begin $display("LEECHCORE_WIRE_DATA_FAIL frame=%0d w0=%h w1=%h",fi,rx_words[before_count],rx_words[before_count+1]);errors=errors+1;end
  end else if(rx_count!=before_count) begin $display("FILTER_FAIL frame=%0d delta=%0d",fi,rx_count-before_count);errors=errors+1;end
  if(fi==4) begin rst=1;repeat(5) @(posedge clk);rst=0;repeat(5) @(posedge clk);end
 end
 if(errors==0)$display("UDP_RX_TEST_PASS");else $display("UDP_RX_TEST_FAIL errors=%0d",errors);$finish;
end
endmodule
